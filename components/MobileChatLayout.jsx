...
const MobileChatLayout = ({ friends, session, sidebarOptions, unseenRequestCount }) => {
    ...
    <div className='fixed bg-zinc-50 border-b border-zinc-200 top-0 inset-x-0 py-2 px-4'>
      <div className='w-full flex justify-between items-center'>
        <Link
          href='/dashboard'
          className=''>
          <Image src={'/icons/logo-no-background.svg'} height={40} width={40} alt='logo' />
        </Link>
        <button onClick={() => setOpen(true)} className='gap-2 flex items-center border border-zinc-200 bg-indigo-600 rounded-lg p-2'>
          <span className='text-white'>Menu</span> 
          <Menu className='h-6 w-6 inline-block text-white' />
        </button>
      </div>
